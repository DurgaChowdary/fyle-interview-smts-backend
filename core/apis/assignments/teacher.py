...
def list_teacher_assignments(principal):
    """ This function returns a list of all the assignments submitted to a specific teacher. """
...
def grade_assignment(principal, incoming_payload):
    """ This functions allows a teacher to grade an assignment. """
    grade_assignment_payload = AssignmentGradeSchema().load(incoming_payload)

    graded_assignment = Assignment.grade_assignment(_id=grade_assignment_payload.id, grade=grade_assignment_payload.grade, principal=principal)
    db.session.commit()
